...
### 1. 前端开发 (client)
- [x] **登录模块**：基于 JWT 的认证流程，包含受保护路由拦截（React Router）。
- [x] **工作台 (Dashboard)**：使用 Recharts 实现数据可视化，统计课程、学生分布及活跃度。
- [x] **课程管理**：实现课程列表展示、状态切换（发布/草稿）、增删改查及筛选。
- [x] **学生管理**：全功能 CRUD 接口对接，支持分页、班级筛选、关键字搜索及选课关联。
- [x] **学习总结**：动态加载并渲染 Markdown 内容，支持代码高亮与静态图片显示。

### 2. 后端开发 (server)
- [x] **Koa 架构搭建**：采用洋葱模型中间件，包含统一错误处理、CORS 及 BodyParser。
- [x] **数据库设计**：使用 SQLite (better-sqlite3) 存储用户、课程、学生及学习记录数据。
- [x] **认证中间件**：实现 JWT 签发与 Bearer Token 验证，保护敏感 API 路由。
- [x] **RESTful API**：设计并实现了符合规范的课程、学生、工作台数据及总结内容接口。
- [x] **静态服务**：配置 Koa 托管前端构建产物及服务端资源文件。

### 3. 文档与工程化
- [x] **API 文档 (`server/API.md`)**：定义统一响应格式及各模块接口规范。
- [x] **阶段总结 (`server/data/summary.md`)**：深度整理前端全栈开发的知识体系与实践思考。
- [x] **工程化配置**：Vite 代理配置、TypeScript 类型约束、Git 版本控制规范。
