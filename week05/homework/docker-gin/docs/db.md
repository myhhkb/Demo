...
| 主表 | 从表 | 关系 | 关联字段 | 业务含义 |
|------|------|------|----------|----------|
| `users` | `words` | 一对多 | `words.user_id` -> `users.id` | 一个用户可以保存多个单词记录 |
...
|------|------|
| 主键 | `users.id` 是用户表主键 |
| 外键 | `words.user_id` 引用 `users.id` |
| 删除策略 | `ON DELETE CASCADE`，用户删除后其单词记录同步删除 |
| 查询范围 | 单词查询、列表、删除操作均按当前登录用户的 `user_id` 过滤 |
| 软删除 | `words.deleted_at` 不为空表示该单词记录已被软删除 |

### 业务示例

| users.id | users.username | words.user_id | words.word | 说明 |
|----------|----------------|---------------|------------|------|
| 1 | `alice` | 1 | `apple` | `alice` 保存了 `apple` |
| 1 | `alice` | 1 | `banana` | `alice` 保存了 `banana` |
| 2 | `bob` | 2 | `orange` | `bob` 保存了 `orange` |

通过上述关系可以看出：同一个用户可以拥有多条单词记录，不同用户之间的单词本互相隔离。
